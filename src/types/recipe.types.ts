...
import { MWObject } from "./item.types";
...
export class Tool {
    ...
    constructor(data: string[]) {
        ...
        this.name = data[1];
        this.proficiency = Number(data[2])
        this.focus = Number(data[3])
        // Extract the special ability for this tool (if any)
        this.dabHandChance = data[5] == "Dab Hand"? Number(data[4]) / 100 : 0;
        this.recycleChance = data[5] == "Recycle"? Number(data[4]) / 100 : 0;
    }
...
export class Supplement {
    ...
    constructor(data: string[]) {
        ...
        this.name = data[0];
        this.proficiency = Number(data[1]);
        this.focus = Number(data[2]);
        this.dabHandChance = data[4] == "Dab Hand"? Number(data[3]) / 100 : 0;
        this.recycleChance = data[4] == "Recycle"? Number(data[3]) / 100 : 0;
...
export class Artisan {
    ...
    constructor(data: string[]) {
        ...
        this.name = data[1];
        this.rarity = data[2];
        this.dabHandChance = data[3] == "Dab Hand"? Number(data[4]) / 100 : 0;
        this.recycleChance = data[3] == "Recycle"? Number(data[4]) / 100: 0;
        this.proficiency = Number(data[7]);
        this.focus = Number(data[8]);
    }
...
export class MWRecipe {
    ...
    /** Multiple the contents of this recipe by a number. */
    ...
    static prettyPrintList(input: [MWRecipe, number][]) {
        ...
        input.forEach((recipeRank: [MWRecipe, number]) => {
            ...
            printString += ` (${Math.round(recipe.normalResults)} Normal,`;
            printString += ` ${Math.round(recipe.highQualityResults)} +1):`;
    ...
    prettyPrint() {
        ...
        for(var supplementMatEntry of this.supplementMaterials) {
            ...
            let supObject = findMwObject(supplementMatEntry[1]);
            if(supObject != undefined && supObject.price != undefined) {
                supplementCost += supplementMatEntry[0] * supObject.price;
            }
        }
